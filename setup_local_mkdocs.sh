...
# MkDocs Local Development Setup Script (macOS/Linux)
...
pip install mkdocs mkdocs-material \
    ...
    mkdocs-material-mark-as-read mkdocs-include-markdown-plugin
...
# Clone the track content repository (which has its own mkdocs.yml and docs/ directory)
git clone https://github.com/40docs/$TRACK.git content
# Move all files from content to the current directory
mv content/* .
mv content/.* . 2>/dev/null || true  # Move hidden files, ignore errors
rm -rf content  # Remove the now-empty content directory

# Clone the theme repository (matching the GitHub Actions workflow)
git clone https://github.com/40docs/workshop_theme_template theme

# === 4. Install additional requirements from content repo ===
echo "=== Installing content-specific requirements ==="

# If the content repo has its own requirements.txt, install those too
if [ -f "requirements.txt" ]; then
    echo "Found requirements.txt, installing additional dependencies..."
    pip install -r requirements.txt
fi

# === 5. Minor adjustments for local development ===
echo "=== Adjusting for local development ==="

# Add landing-page.css to docs/ if it doesn't exist
if [ ! -f docs/landing-page.css ]; then
